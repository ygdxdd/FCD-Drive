...
class AbstractAgent(torch.nn.Module, ABC):
    ...
    def compute_trajectory(self, agent_input: AgentInput) -> Trajectory:
        ...
        with torch.no_grad():
            ...
            #poses = predictions["trajectory"][0].squeeze(0).numpy()
            poses = predictions["trajectory"][0].numpy()
            poses1 = predictions["trajectory"][1].numpy()
            poses2 = predictions["trajectory"][2].numpy()
            poses3 = predictions["trajectory"][3].numpy()
            poses4 = predictions["trajectory"][4].numpy()

        # extract trajectory
        #return Trajectory(poses)
        return Trajectory(poses), Trajectory(poses1), Trajectory(poses2), Trajectory(poses3), Trajectory(poses4)
