...
def transfuser_loss(
    ...
    if "trajectory_loss" in predictions:
        ...
        target_traj = norm_odo(targets["trajectory"])
        predictions_traj = norm_odo(predictions["trajectory"])
        trajectory_loss = F.l1_loss(predictions_traj[..., :2], target_traj[..., :2])
...
def _agent_loss(
    ...
    l1_cost = _get_l1_cost(gt_states, pred_states, gt_valid)#l1loss
