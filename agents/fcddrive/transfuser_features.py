...
class TransfuserFeatureBuilder(AbstractFeatureBuilder):
    ...
    def numpy_bilinear_resize(self, src, dsize):
        """
        纯numpy实现双线性插值图像缩放，兼容所有numpy数组，输出和cv2.resize(interp=LINEAR)一致
        :param src: 输入numpy数组 (H, W, C)
        :param dsize: 目标尺寸 (w, h)，和cv2.resize参数一致
        :return: 缩放后的numpy数组 (h, w, C)，uint8类型，连续内存
        """
        ...
        # 创建输出数组
        dst = np.zeros((h_dst, w_dst, c_src), dtype=np.uint8)
        ...
        for y_dst in range(h_dst):
            for x_dst in range(w_dst):
                ...
                for c in range(c_src):
                    dst[y_dst, x_dst, c] = np.uint8(
                        wy0 * (wx0 * src[y0, x0, c] + wx1 * src[y0, x1, c]) +
                        wy1 * (wx0 * src[y1, x0, c] + wx1 * src[y1, x1, c])
                    )
        # 确保连续内存，兼容后续逻辑
    ...
    def _get_camera_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        #print(f"DEBUG_get_images: agent_input type = {type(agent_input)}")
        #print(f"DEBUG_get_images: agent_input dir = {dir(agent_input)}")
        # stitch l0, f0, r0 images
        stitched_image = np.concatenate([l0, f0, r0], axis=1)
        #print(f"DEBUG: Concatenation successful, shape: {stitched_image.shape}")
        #print(f"DEBUG_get_images: l0 type = {type(l0)}")
        #print(f"DEBUG_get_images: l0  = {l0}")
        #print(f"DEBUG_get_images: f0 type = {type(f0)}")
        #print(f"DEBUG_get_images: f0 = {f0}")
        #print(f"DEBUG_get_images: r0 type = {type(r0)}")

        '''
        print("=== 强制校验stitched_image ===")
        print(f"类型: {type(stitched_image)}, 是否为None: {stitched_image is None}")
        if isinstance(stitched_image, np.ndarray): 
            print(f"转换前内存是否连续: {stitched_image.flags.contiguous}") 
            print(f"是numpy数组，形状: {stitched_image.shape}")
            stitched_image = np.ascontiguousarray(stitched_image)
            print(f"转换后内存是否连续: {stitched_image.flags.contiguous}")
        '''
        resized_image = self.numpy_bilinear_resize(stitched_image, (1024, 256))
