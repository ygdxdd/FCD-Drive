...
class TransfuserAgent(AbstractAgent):
    ...
    def unfreeze_perception_layers(self):
        """解冻感知层 - 调用模型的解冻方法"""
        print("[TransfuserAgent] 开始解冻感知层...")
        
        self._transfuser_model.unfreeze_perception_layers()
    
    def freeze_perception_layers(self):
        """冻结感知层 - 调用模型的冻结方法"""
        print("[TransfuserAgent] 开始冻结感知层...")
        
        self._transfuser_model.freeze_perception_layers()
    
    def unfreeze_planner_layers(self):
        """解冻规划层 - 调用模型的解冻方法"""
        print("[TransfuserAgent] 开始解冻规划层...")
        
        self._transfuser_model.unfreeze_planner_layers()
    
    def freeze_planner_layers(self):
        """冻结规划层 - 调用模型的冻结方法"""
        print("[TransfuserAgent] 开始冻结规划层...")
        
        self._transfuser_model.freeze_planner_layers()        
    
    def unfreeze_diff_layers(self):
        """解冻扩散层 - 调用模型的解冻方法"""
        print("[TransfuserAgent] 开始解冻扩散层...")
        
        self._transfuser_model.unfreeze_diff_layers()
    
    def freeze_diff_layers(self):
        """冻结扩散层 - 调用模型的冻结方法"""
        print("[TransfuserAgent] 开始冻结扩散层...")
        
        self._transfuser_model.freeze_diff_layers()
