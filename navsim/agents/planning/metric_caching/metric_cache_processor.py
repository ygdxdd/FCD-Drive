...
class MetricCacheProcessor:
    ...
    def _interpolate_gt_observation(self, scenario: AbstractScenario) -> PDMObservation:
        ...
        object_appearance_count = {}  # 跟踪每个对象出现的次数
        ...
        for time_s, detection_track in zip(relative_time_s, gt_detection_tracks):
            ...
            # create time interpolators (只对有多于1个数据点的对象创建插值器)
        ...
        # 记录统计信息（可选）
        if skipped_objects:
            print(f"INFO: Skipped {len(skipped_objects)} single-point objects in scenario {scenario.scenario_name}")
        # interpolate at 10Hz
        ...
        for time_s in interpolated_time_s:
          ...
          # 添加被跳过的单点对象（保持原始状态）
            for token in skipped_objects:
                if token in unique_detection_tracks:
                    interpolated_tracks.append(unique_detection_tracks[token])
